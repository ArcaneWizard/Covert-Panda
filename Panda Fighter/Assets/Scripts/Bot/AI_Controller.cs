...
public class AI_Controller : MonoBehaviour
{
    ...
    private AI_WeaponAttacks weaponAttacks;
    private AI_AnimationController controller;
    ...
    void Awake()
    {
        ...
        weaponAttacks = transform.GetComponent<AI_WeaponAttacks>();
        controller = transform.GetComponent<AI_AnimationController>();
    ...
    private void lookAndAimInRightDirection()
    {
        ...
        if (!controller.disableLimbsDuringDoubleJump)
        {
            ...
            if (Input.mousePosition.x >= camera.WorldToScreenPoint(shootingArm.parent.position).x)
                alien.localRotation = Quaternion.Euler(0, 0, 0);
            else
                alien.localRotation = Quaternion.Euler(0, 180, 0);
            ...
            zAngle *= (alien.localEulerAngles.y / 90 - 1) * Mathf.Sign(transform.eulerAngles.z - 180);
            shootAngle -= zAngle;
    ...
    public IEnumerator isGrounded()
    {
        ...
        if (grounded)
        {
            ...
            if (((movementDirX == 1 && alien.localEulerAngles.y == 0) || (movementDirX == -1 && alien.localEulerAngles.y == 180)) && rightFootGround)
            {
            ...
            else if (((movementDirX == -1 && alien.localEulerAngles.y == 0) || (movementDirX == 1 && alien.localEulerAngles.y == 180)) && leftFootGround)
            {
